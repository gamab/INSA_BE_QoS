...
import processor.InviteOkProcessor;
...
public class Proxy extends UnicastRemoteObject implements SipListener, RemoteServiceInterface, Runnable
{
	...
	private boolean verifRessources = false;
	private Controller cont;         

	/**
	 * Request Processor is going to extract the interesting data from INVITE and OK
	 */
	private InviteOkProcessor reqProc;
	...
	private void processIncomingResponse(ResponseEvent responseEvent) throws InvalidArgumentException, ParseException, SipException
	{
		...
		this.reqProc.processOk(response, cseqHeader, statusCode,cont);
		
		if (getVerifRessources())
	    {
				System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!! [Ressources OK; Send OK to clients SIP] !!!!!!!!!!!!!!!!!!!!!!!!!!");
				if (log.isDebugEnabled())
				{
					log.debug("-------------------");
		...
		else
		{
			...
			try 
			{
				...
				SipFactory sipFactory = SipFactory.getInstance();
				properties.setProperty("javax.sip.IP_ADDRESS","192.168.1.100");
				properties.setProperty("javax.sip.STACK_NAME","QOS_BE[bye]");
				properties.setProperty("javax.sip.OUTBOUND_PROXY","192.168.1.100:5060/UDP");
